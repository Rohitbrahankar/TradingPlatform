...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white relative">
      ...
      <section className="absolute bottom-5 right-5 z-40 flex flex-col justify-end items-end gap-2">
        {isBotRelease && (
          <div className="rounded-md w-[20rem] md:w-[25rem] lg:w-[25rem] h-[70vh] bg-gray-900">
            <div className="flex justify-between items-center border-b px-6 h-[12%]">
              <p className="text-wh">Chat Bot</p>
              <Button onClick={handleBotRelease} size="icon" variant="ghost">
                <Cross1Icon />
              </Button>
            </div>

            <div className="h-[76%] flex flex-col overflow-y-auto gap-3 px-5 py-2">
              <div className="self-start pb-5 w-auto">
                <div className="justify-end self-end px-5 py-2 rounded-md bg-black w-auto">
                  {`hi, ${auth.user?.fullName}`}
                  <p>you can ask crypto related any question</p>
                  <p>like, price, market cap extra...</p>
                </div>
              ...
              {chatBot.messages.map((item, index) => (
                ...
                >
                  {item.role === "user" ? (
                    <div className="px-5 py-2 rounded-full bg-gray-800 w-auto">
                      {item.prompt}
                    </div>
                  ) : (
                    <div className="w-full">
                      <div className="bg-gray-700 flex gap-2 py-4 px-4 rounded-md min-w-[15rem] w-full">
                        <p>{item.ans}</p>
                      </div>
                    </div>
                  )}
            ...
            <div className="h-[12%] border-t">
              <Input
                className="w-full h-full border-none outline-none bg-gray-800 text-white"
                placeholder="write prompt"
                onChange={handleChange}
                value={inputValue}
                onKeyPress={handleKeyPress}
        ...
        <div
          onClick={handleBotRelease}
          className="relative w-[10rem] cursor-pointer group mt-3"
        >
          <Button className="w-full h-[3rem] gap-2 items-center bg-gray-800 hover:bg-gray-700">
            <MessageCircle
              fill=""
              className="fill-gray-800 -rotate-[90deg] stroke-none group-hover:fill-gray-700"
              size={30}
            />
            <span className="text-2xl text-white">Chat Bot</span>
          </Button>
        </div>
      </section>
    </div>
